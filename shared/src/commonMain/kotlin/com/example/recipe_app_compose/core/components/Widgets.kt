...
import com.example.recipe_app_compose.shared.generated.resources.confirm
...
fun ConfirmationDialog(
    ...
    dismissLabel: String = stringResource(Res.string.dismiss),
) {
    AlertDialog(
        title = { Text(title) },
        text = { Text(message) },
...
fun AppMediaCard(
    ...
    Card(
        ...
        Column(Modifier.fillMaxWidth().padding(AppSpacing.Medium), content = content)
    }
}
...
fun AppHorizontalMediaCard(
    ...
    Card(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(Modifier.weight(1f).padding(AppSpacing.Medium), content = content)
        }
    }
}
...
fun ExternalLinkText(
    ...
    val linkText = buildAnnotatedString {
        withLink(LinkAnnotation.Url(url, TextLinkStyles(SpanStyle(color = linkTextColor))) { annotation ->
            uriHandler.openUri((annotation as LinkAnnotation.Url).url)
        }) { append(text) }
    }
    Text(linkText, modifier = modifier, style = style)
}
