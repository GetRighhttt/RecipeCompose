...
class DatabaseRepoImpl(private val randomMealDAO: RandomMealDAO) : DatabaseRepository {
    override suspend fun executeInsertMeal(meal: RandomMeal) =
        withContext(Dispatchers.IO) { randomMealDAO.insertMeal(meal = meal) }

    override suspend fun executeDeleteMeal(meal: RandomMeal) =
        withContext(Dispatchers.IO) { randomMealDAO.deleteMeal(meal = meal) }

    override suspend fun executeGetAllMeals(): Flow<List<RandomMeal>> =
        withContext(Dispatchers.IO) { randomMealDAO.getAllMeals() }

    override suspend fun executeDeleteAll() =
        withContext(Dispatchers.IO) { randomMealDAO.deleteAll() }
}